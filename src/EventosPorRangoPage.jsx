...
export default function EventosPorRangoPage({
  ...
  const changeLang = (newLang) => navigate(`${getLangPrefix(newLang)}/${page}`);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f5f3ef", fontFamily: "system-ui, -apple-system, sans-serif" }}>
      <style>{`
        ...
        .rp-card-meta { display: flex; align-items: flex-start; gap: 6px; color: #555; font-size: 13px; margin: 3px 0; }
      ...
      <nav className="rp-nav">
        <button className="rp-back" onClick={() => window.history.length > 1 ? navigate(-1) : navigate(`${langPrefix}/`)}>
